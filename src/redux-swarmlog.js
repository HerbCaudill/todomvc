...
export default class ReduxSwarmLog {
  ...
  getSwarmLog() {
    return swarmlog({
      ...
      db: this.db,
      valueEncoding: 'json',
      hubs: ['https://signalhub-jccqtwhdwc.now.sh/'],
    })
  }

  startReadStream() {
    this.log.createReadStream({ live: true }).on('data', function(data) {
...
%c${typeof payload === 'string' ? payload : JSON.stringify(payload, null, 2)}
